...
class SentenceEncoder:
    def __init__(self, model_name='paraphrase-distilroberta-base-v1', device='cuda'):
...
sentencoder = SentenceEncoder()
original_sentence = "I have two daughters. Do you have any children?"
generated_sentences = ["I am a mother of two. Have you got kids?", 
                       "I've got two girls. What about you?", 
                       "Two daughters are mine. Do kids brighten your life?"]

#sentencoder = SentenceEncoder()
best_sentence, best_index, best_similarity = sentencoder.find_best_match(original_sentence, generated_sentences)
print(f"Best matching sentence: '{best_sentence}' with similarity: {best_similarity}")
...
# Example|
sentence = "Not really.  We were when I was younger.  I just don't know how she'll feel about my dogs"
salient_words = identify_salient_words(sentence)
print("Salient words:", salient_words)
...
def predict_masked_sentences_for_salient_words(sentence, num_sentences=20, top_k=5):
    berttokenizer = AutoTokenizer.from_pretrained('bert-large-uncased')
    bertmodel = AutoModelForMaskedLM.from_pretrained('bert-large-uncased').eval().to(device)
    salient_words = identify_salient_words(sentence)
...
original_sentence = "Keeping active will make you feel younger and healthy too. Will you teach your grandchild soccer? "
generated_sentences = predict_masked_sentences_for_salient_words(original_sentence, 30)
print(generated_sentences)

config = AutoConfig.from_pretrained("./DGSlow_Bartbase", num_beams=1, num_beam_groups=1)
model = AutoModelForSeq2SeqLM.from_pretrained("./DGSlow_Bartbase")
tokenizer = AutoTokenizer.from_pretrained("./DGSlow_Bartbase")

model = model.to(device)
eos_token = tokenizer.eos_token
#tokenizer = tokenizer.to(deive)

def get_cls_loss( sentence: List[str], labels: List[str]):
    inputs = tokenizer(
            sentence,
            return_tensors = "pt",
            padding = True,
...
original_sentence = "I have two daughters. Do you have any children?"
guided_sentence = "Yes I have a son and just recently i became a grandpa"
rs_demo = get_cls_loss([original_sentence], [guided_sentence])

print(rs_demo)

def initialize_population(ori_text, num_individuals):
    return predict_masked_sentences_for_salient_words(ori_text, num_individuals)

def objective_cls(pop,guided_sentence):
    cls_losses = []
    for sentence in pop:
        #text = context + tokenizer.eos_token + sentence
        cls_loss = get_cls_loss([sentence], [guided_sentence])
...
def remove_pad(s: torch.Tensor):
    return s[torch.nonzero(s != pad_token_id)].squeeze(1)

def compute_seq_len(seq: torch.Tensor):
    if seq.shape[0] == 0: # empty sequence
        return 0
    if seq[0].eq(pad_token_id):
        return int(len(seq) - sum(seq.eq(pad_token_id)))
    else:
        return int(len(seq) - sum(seq.eq(pad_token_id))) - 1


def get_prediction( sentence: Union[str, List[str]]):
        text = sentence
        inputs = tokenizer(
            text,
            return_tensors="pt",
...
#             seqs = outputs['sequences'][:, input_ids.shape[-1]:].detach()
        ...
        pred_len = [compute_seq_len(seq) for seq in seqs]
        return pred_len, seqs, out_scores

def compute_batch_score(text: List[str]):
        batch_size = len(text)
        num_beams =  1
        batch_size = len(text)
        index_list = [i * 1 for i in range(batch_size + 1)]
        pred_len, seqs, out_scores = get_prediction(text)
...
def compute_score(text: Union[str, List[str]], batch_size: int = None):
        total_size = len(text)
        ...
        if batch_size < total_size:
            ...
            for start in range(0, total_size, batch_size):
                ...
                score, seq, p_len = compute_batch_score(text[start: end])
                pred_len.extend(p_len)
                seqs.extend(seq)
                scores.extend(score)
        else:
            scores, seqs, pred_len = compute_batch_score(text)
        return scores, seqs, pred_len
def leave_eos_target_loss(scores: list, seqs: list, pred_len: list):
        loss = []
        for i, s in enumerate(scores): # s: T X V
            if pred_len[i] == 0:
                loss.append(torch.tensor(0.0, requires_grad=True).to(device))
            else:
                s[:,pad_token_id] = 1e-12
                softmax_v = softmax(s)
                eos_p = softmax_v[:pred_len[i],eos_token_id]
...
def objective_eos(pop,guided_sentence):
    eos_losses = []
    for sentence in pop:
        #text = context + tokenizer.eos_token + sentence
        scores, seqs, pred_len = compute_score([sentence])
        eos_loss = leave_eos_target_loss(scores, seqs, pred_len)
...
def mutation(pop):
    ...
    for sentence in pop:
        ...
        mutated_sentences = predict_masked_sentences_for_salient_words(sentence,10)
...
def POPOP(
    ...
    pop = initialize_population(sentence, num_individuals)
    #print(pop)
...
original_sentence = "Not really.  We were when I was younger.  I just don't know how she'll feel about my dogs"
pop = initialize_population(original_sentence, 10)

print(pop)

print(objective_cls(pop,guided_sentence))
context = "<PS>i am a grandparent at 44.<SEP>what do you make there ? cars , boats , planes or goods and sprockets ?<SEP>i build commercial planes i work the night shift . do you have any kids<SEP>I have two daughters. Do you have any children?<SEP>Yes I have a son and just recently i became a grandpa<SEP>Congratulations! That's great. Do you have any hobbies?<SEP>Thank you! I try to not feel old haha. Yes I love playing soccers with my friends quite often. "
original_sentence = "Keeping active will make you feel younger and healthy too. Will you teach your grandchild soccer?"
guided_sentence = "That is so right. It feels so much better than just becoming passive when getting older. Of course I will, keeping the heritage"

pop, pop_fitness, best_fitness = POPOP(context, original_sentence, guided_sentence, objective_cls, tournament_selection ,num_individuals=10, max_evaluations=20)

print("DAY LA KET QUA CHAY THU:",pop)
    ...
    def generation_step(self, instance: dict):
        ...
        for entry_idx in range(num_entries):
            ...
            if args.objective == "cls":
                pop, pop_fitness, best_fitness = POPOP(original_context, free_message, guided_message, objective_cls, tournament_selection ,num_individuals= args.num_ind, max_evaluations= 5 * args.num_ind)
            else:
                pop, pop_fitness, best_fitness = POPOP(original_context, free_message, guided_message, objective_eos, tournament_selection ,num_individuals= args.num_ind, max_evaluations= 5 * args.num_ind)
            ...
            cos_sim = sentencoder.get_sim(best_individual, free_message)
