...
class DGAttackEval(DGDataset):
    ...
    def generation_step(self, instance: dict):
        ...
        for entry_idx in range(num_entries):
            ...
            problem = nsga2_clm.Problem(self.model, self.tokenizer,original_context, free_message, guided_message, self.device,args.max_len,self.task,args.acc_metric,self.bleu,self.rouge,self.meteor,output)

            evolution = nsga2_clm.Evolution(args.crossover_flag, self.write_file_path, problem, num_of_generations=args.num_gen, num_of_individuals=args.num_ind, num_of_tour_particips=2,
            ...
            data_with_fitness = [(sentence, accuracy, length, (length + 1) / (accuracy + 1)) for sentence, accuracy, length in result]
